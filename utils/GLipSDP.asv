function [Lip,info,time] = GLipSDP(NN)
...
for ii = 1:depth-1
    ...
    switch layer
        case 'conv'
            ...
            [con, Qm] = get_con_conv(W{ii}, strides_ii, Qm, 0, pool{ii}, glob_loc, Alpha{ii}, Beta{ii});
        case 'fc'
            if ii>=2
            ...
            [con, Qm] = get_con_fc(W{ii}, Qm, 0, glob_loc, Alpha{ii}, Beta{ii});
        case 'subn_conv'
            strides_ii = [strides(ii),pool_kernel(ii),pool_strides(ii)];
        ...
        case 'res_conv2'
            strides_ii = [strides(ii),pool_kernel(ii),pool_strides(ii)];
            [con, Qm] = get_con_res_conv2(W{ii}, strides_ii, Qm, pool{ii}, 0, glob_loc, Alpha{ii}, Beta{ii});
        case 'res_fc2'
            [con, Qm] = get_con_res_fc2(W{ii}, Qm, 0, glob_loc, Alpha{ii}, Beta{ii});
    end
    cons = [cons,con];
end
...
function [con,Q] = get_con_conv(K, strides, Qm, last, pool, glob_loc, Alpha, Beta)
...
if last == 1
    ...
    switch glob_loc
        ...
        case 'local'
            con = [[P-A'*P*A+2*C'*Alpha*Beta*Lambda*C -A'*P*B+2*C'*Alpha*Beta*Lambda*D -C'*(Alpha+Beta)*Lambda;...
                -B'*P*A+2*D'*Alpha*Beta*Lambda*C Qm-B'*P*B+2*D'*Alpha*Beta*Lambda*D -D'*(Alpha+Beta)*Lambda;...
                -Lambda*(Alpha+Beta)*C -Lambda*(Alpha+Beta)*D 2*Lambda-Q]>=1E-10, P>=1E-10, Lambda>=0];
    end
end
end

function [con,Q] = get_con_fc(W,Qm,last,glob_loc, Alpha, Beta)
[ny,nu] = size(W);
...
if last == 1
    ...
    switch glob_loc
        ...
        case 'local'
            con = [[Qm+2*W'*Alpha*Beta*Lam*W -W'*(Alpha+Beta)*Lam; -Lam*(Alpha+Beta)*W 2*Lam-Q]>=0,lam>=0];
    end
end
end
...
function [cons,Q] = get_con_res_fc2(W, Qm, last, glob_loc, Alpha, Beta)

if nargin < 4 || isempty(glob_loc)
    glob_loc = 'global';
end
...
if last == 1
    Q = [];
    cons = [[Qm-eye(nu) -W{1}'*Lambda-W{2};...
        -Lambda*W{1}-W{2}' 2*Lambda-W{2}'*W{2}]>=10e-10, lam>=0];
else
    Q = sdpvar(ny,ny);
    switch glob_loc
        case 'global'
            cons = [[Qm-Q -W{1}'*Lambda-Q*W{2};...
                -Lambda*W{1}-W{2}'*Q 2*Lambda-W{2}'*Q*W{2}]>=10e-10, lam>=0];
        case 'local'
            cons = [[Qm-Q+2*W{1}'*Alpha*Beta*Lambda*W{1} -W{1}'*(Alpha+Beta)*Lambda-Q*W{2};...
                -Lambda*(Alpha+Beta)*W{1}-W{2}'*Q 2*Lambda-W{2}'*Q*W{2}]>=10e-10,lam>=0];
    end
end
end

function [cons,Q] = get_con_res_conv2(W, strides, Qm, pool, last, glob_loc, Alpha, Beta)

if nargin < 6 || isempty(glob_loc)
    glob_loc = 'global';
end
...
if last == 1
    ...
    cons = [[Qm-B1'*P1*B1-eye(nu1) -B1'*P1*A1     -D1'*Lambda-D2               -C2;...
        -A1'*P1*B1             P1-A1'*P1*A1   -C1'*Lambda                  zeros(nx1,nx2);...
        -Lambda*D1-D2'         -Lambda*C1     2*Lambda-B2'*P2*B2-D2'*D2    -B2'*P2*A2-D2'*C2;...
        -C2'                   zeros(nx2,nx1) -A2'*P2*B2-C2'*D2            P2-A2'*P2*A2-C2'*C2]>=10e-10,P1>=10e-10,P2>=10e-10,Lambda>=0];
else
    [Q,Qm] = get_Q_conv(Qm,ny2,pool,strides);
    switch glob_loc
        case 'global'
            cons = [[Qm-B1'*P1*B1-Q  -B1'*P1*A1     -D1'*Lambda-Q*D2             -Q*C2;...
                -A1'*P1*B1       P1-A1'*P1*A1   -C1'*Lambda                  zeros(nx1,nx2);...
                -Lambda*D1-D2'*Q -Lambda*C1     2*Lambda-B2'*P2*B2-D2'*Q*D2  -B2'*P2*A2-D2'*Q*C2;...
                -C2'*Q           zeros(nx2,nx1) -A2'*P2*B2-C2'*Q*D2          P2-A2'*P2*A2-C2'*Q*C2]>=10e-10,P1>=10e-10,P2>=10e-10,Lambda>=0];
        case 'local'
            cons = [[Qm-B1'*P1*B1-Q+2*D1'*Alpha*Beta*Lambda*D1, ...
                -B1'*P1*A1+2*D1'*Alpha*Beta*Lambda*C1, ...
                -D1'*(Alpha+Beta)*Lambda-Q*D2, ...
                -Q*C2; ...
                -A1'*P1*B1+2*C1'*Alpha*Beta*Lambda*D1, ...
                P1-A1'*P1*A1+2*C1'*Alpha*Beta*Lambda*C1, ...
                -C1'*(Alpha+Beta)*Lambda, ...
                zeros(nx1,nx2); ...
                -Lambda*(Alpha+Beta)*D1-D2'*Q, ...
                -Lambda*(Alpha+Beta)*C1, ...
                2*Lambda-B2'*P2*B2-D2'*Q*D2, ...
                -B2'*P2*A2-D2'*Q*C2; ...
                -C2'*Q, ...
                zeros(nx2,nx1), ...
                -A2'*P2*B2-C2'*Q*D2, ...
                P2-A2'*P2*A2-C2'*Q*C2]>=10e-10,P1>=10e-10,P2>=10e-10,Lambda>=0];
    end
end
